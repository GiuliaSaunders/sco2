...
public class ManterColaborador extends ManageBeanGeral<Colaborador> {
	...
	private transient ManageProduto manageProduto;
	...
	protected List<ValidationRequest> getCamposObrigatorios() {
		...
		if (getTipoPessoa().isPf()) {
			obrigatorios
					.add(new ValidationRequest("cpf", ValidatorFactory.newStrEmpty(), "formColaborador:entradaCpf"));
		} else if (getTipoPessoa().isPj()) {
			obrigatorios
					.add(new ValidationRequest("cnpj", ValidatorFactory.newStrEmpty(), "formColaborador:entradaCnpj"));
		}
		return obrigatorios;
	}
	...
	protected List<ValidationRequest> getCamposValidados() {
		...
		validados.add(new ValidationRequest("observacao", ValidatorFactory.newStrMaxLen(500, true),
				"formColaborador:inTextBoxObs", allowed));
		validados.add(
				new ValidationRequest("email", ValidatorFactory.newEmail(45), "formColaborador:entradaEmail", allowed));
		validados.add(new ValidationRequest("nome", ValidatorFactory.newStrRangeLen(100, 4, true),
				"formColaborador:entradaNome"));
		validados.add(new ValidationRequest("endereco", ValidatorFactory.newStrRangeLen(150, 4, true),
				"formColaborador:entradaEndereco", allowed));
		validados.add(new ValidationRequest("cidade", ValidatorFactory.newStrRangeLen(45, 3, true),
				"formColaborador:entradaCidade", allowed));
		if (getTipoPessoa().isPf()) {
