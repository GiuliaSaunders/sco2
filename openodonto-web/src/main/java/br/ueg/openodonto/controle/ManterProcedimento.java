...
public class ManterProcedimento extends ManageBeanGeral<Procedimento> {
	...
	protected List<ValidationRequest> getCamposObrigatorios() {
		...
		obrigatorios
				.add(new ValidationRequest("valor", ValidatorFactory.newNumMin(0), "formProcedimento:entradaValor"));
		return obrigatorios;
	}
	...
	protected List<ValidationRequest> getCamposValidados() {
		...
		validados.add(new ValidationRequest("nome", ValidatorFactory.newStrRangeLen(150, 4, true),
				"formProcedimento:entradaNome"));
		validados.add(new ValidationRequest("descricao", ValidatorFactory.newStrMaxLen(300, true),
				"formProcedimento:inTextBoxObs", allowed));
		return validados;
	}
